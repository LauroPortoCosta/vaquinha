unit va_03_debitos2;

interface

uses
  System.SysUtils, System.Types, System.UITypes, System.Classes, System.Variants,
  FMX.Types, FMX.Controls, FMX.Forms, FMX.Graphics, FMX.Dialogs, FMX.Objects,
  FMX.Layouts, FMX.Controls.Presentation, FMX.StdCtrls, FMX.ListBox, System.JSON,
  FMX.Edit,
  FMX.ListView,
  FMX.ListView.Types,
  FMX.ListView.Appearances,
  FMX.ListView.Adapters.Base, FMX.Ani,
  FireDAC.Comp.Client, FireDAC.DApt,
  FireDAC.Comp.DataSet, REST.Types, FMX.Memo.Types,
  FMX.ScrollBox, FMX.Memo;


type
  Tva_debitos2 = class(TForm)
    Lay01_cabecalho: TLayout;
    Rect1_cabecalho: TRectangle;
    Lay2_direito: TLayout;
    Img2_direita: TImage;
    Lay1_esquerdo: TLayout;
    Img1_esquerda: TImage;
    lbl: TLabel;
    Rect_Central: TRectangle;
    Lay02_saldo: TLayout;
    Rectangle2: TRectangle;
    Lay1_ListBox: TLayout;
    Rect_ListBox: TRectangle;
    Lay2_Valor: TLayout;
    Rect_do_Valor1: TRectangle;
    Lay3: TLayout;
    Label11: TLabel;
    Rectangle8: TRectangle;
    Lay4: TLayout;
    Rectangle9: TRectangle;
    Label13: TLabel;
    Rectangle10: TRectangle;
    Image5: TImage;
    Label14: TLabel;
    Label15: TLabel;
    Lay06_rodape: TLayout;
    RectX_rodape: TRectangle;
    Label16: TLabel;
    lb_icone: TListBox;
    ListBoxItem2: TListBoxItem;
    ImageA2: TImage;
    ListBoxItem3: TListBoxItem;
    ImageA3: TImage;
    ListBoxItem4: TListBoxItem;
    ListBoxItem5: TListBoxItem;
    ImageB1: TImage;
    ListBoxItem6: TListBoxItem;
    ImageB2: TImage;
    ListBoxItem7: TListBoxItem;
    ImageB3: TImage;
    ListBoxItem8: TListBoxItem;
    ImageB4: TImage;
    ListBoxItem9: TListBoxItem;
    ImageC1: TImage;
    ListBoxItem10: TListBoxItem;
    ImageC2: TImage;
    ListBoxItem11: TListBoxItem;
    ImageC3: TImage;
    ListBoxItem12: TListBoxItem;
    ImageC4: TImage;
    ListBoxItem13: TListBoxItem;
    ImageD1: TImage;
    ListBoxItem14: TListBoxItem;
    ImageD2: TImage;
    ListBoxItem15: TListBoxItem;
    ImageD3: TImage;
    ListBoxItem16: TListBoxItem;
    ImageD4: TImage;
    LabelC2: TLabel;
    LabelC4: TLabel;
    LabelC3: TLabel;
    LabelD1: TLabel;
    LabelA2: TLabel;
    LabelA3: TLabel;
    LabelD2: TLabel;
    LabelD3: TLabel;
    LabelD4: TLabel;
    LabelA4: TLabel;
    LabelB1: TLabel;
    LabelB2: TLabel;
    LabelB3: TLabel;
    LabelC1: TLabel;
    LabelB4: TLabel;
    ListBoxItem1: TListBoxItem;
    img_selecao: TImage;
    ImageA1: TImage;
    lbCategoria: TListBox;
    ImageA4: TImage;
    Edit_VALOR: TEdit;
    Label_listBox: TLabel;
    ListView1: TListView;
    ListBox1: TListBox;
    ListBoxItem17: TListBoxItem;
    Image1: TImage;
    ImageXA1: TImage;
    ListBoxItem18: TListBoxItem;
    ImageXA2: TImage;
    ListBoxItem19: TListBoxItem;
    ImageXA3: TImage;
    ListBoxItem20: TListBoxItem;
    ImageXA4: TImage;
    ListBoxItem21: TListBoxItem;
    ImageXB1: TImage;
    ListBoxItem22: TListBoxItem;
    ImageXB2: TImage;
    ListBoxItem23: TListBoxItem;
    ImageXB3: TImage;
    ListBoxItem24: TListBoxItem;
    ImageXB4: TImage;
    ListBoxItem25: TListBoxItem;
    ImageXC1: TImage;
    ListBoxItem26: TListBoxItem;
    ImageXC2: TImage;
    ListBoxItem27: TListBoxItem;
    ImageXC3: TImage;
    ListBoxItem28: TListBoxItem;
    ImageXC4: TImage;
    ListBoxItem29: TListBoxItem;
    ImageXD1: TImage;
    ListBoxItem30: TListBoxItem;
    ImageXD2: TImage;
    ListBoxItem31: TListBoxItem;
    ImageXD3: TImage;
    ListBoxItem32: TListBoxItem;
    ImageXD4: TImage;
    ArcDial1: TArcDial;
    Edit1_VALOR_PAR: TEdit;
    Edit_DESCRIAO: TEdit;
    Rect_do_Valor2: TRectangle;
    Line1: TLine;
    Image2: TImage;
    Image3: TImage;
    RoundRect3: TRoundRect;
    AnimationLarg: TFloatAnimation;
    lbl_arc: TLabel;
    procedure Img2_direitaClick(Sender: TObject);
    procedure ImageA1Click(Sender: TObject);
    procedure FormCreate(Sender: TObject);
    procedure lbCategoriaItemClick(const Sender: TCustomListBox;    const Item: TListBoxItem);
    procedure Edit_VALORTyping(Sender: TObject);
    procedure FormShow(Sender: TObject);
    procedure ListView1ItemClick(const Sender: TObject;    const AItem: TListViewItem);
    procedure Edit1_VALOR_PARTyping(Sender: TObject);
    procedure Rectangle9DblClick(Sender: TObject);
    procedure img_addClick(Sender: TObject);
    procedure FormClose(Sender: TObject; var Action: TCloseAction);
    procedure RoundRect3Click(Sender: TObject);
    procedure resolve;
  private

    icone_selecionado  : TBitmap;                                               // USADA NA  ==>  procedure img_saveClick(Sender: TObject); E NA procedure SelecionaIcone(img: TImage)
    indice_selecionado : Integer;                                               // USADA NA   ==>  procedure SelecionaIcone(img: TImage)

    id_img       : String;
    id           : String;   // cuidado com id .... deu problema
    id_usuario   : String;
    meu_grupo    : String;
    categoria_u  : Integer;   // PosicionaCategoria - indica que é a categoria que deve ser selecionada
    grupo_u      : Integer;   // PosicionaGrupo - com este valor o sistema posiciona na tela o grupo informado
    numerador    : integer;  // numerador := va_05_dm.DM.FDMemTable_contador.FieldByName('lancamento').AsInteger;

    procedure SelecionaIcone(img: TImage);
    procedure ListarCategorias;
    procedure AddCategoria(id,icone: integer; descricao: string);
    procedure Carrega;
    procedure addcategoria2(var1, var2, var3: string);
    procedure PosicionaCategoria;
    procedure PosicionaGrupo;
    procedure Prepara;
    procedure Alteracao;
    procedure Inclusao;
    { Private declarations }
  public
  descricao : string;
    { Public declarations }
  end;
//---------------------------------------------------------------------------------------//
/////////////////////////////  I N C L U S Ã O   /////////////////////////////////////////
//--------------------------------------------------------------------------------------//

var
  va_debitos2: Tva_debitos2;

implementation

{$R *.fmx}

uses va_05_dm, uFormat, cLancamento, cUsuario, UnitLogin, va_01_abertura;

procedure Tva_debitos2.Edit1_VALOR_PARTyping(Sender: TObject);
begin
//  Formatar(Edit1_VALOR_PAR, TFormato.Valor);                                    //procedure Formatar(Obj: TObject; Formato : TFormato; Extra : string = ''); esta no uFormat
end;


procedure Tva_debitos2.Edit_VALORTyping(Sender: TObject);
begin
//  Formatar(Edit_VALOR, TFormato.Valor);                                         //procedure Formatar(Obj: TObject; Formato : TFormato; Extra : string = ''); esta no uFormat
end;


procedure Tva_debitos2.FormClose(Sender: TObject; var Action: TCloseAction);
var  // Ao sair do formulario , GRAVE o GRUPO e CATEGORIA que voce estava usando
    u    : TUsuario ;
    erro : string   ;
    cc   : String   ;
 begin
      try
         u              := TUsuario.Create(va_05_dm.DM.conn) ;
         u.ID_USUARIO   := StrToInt(id_usuario)  ;
         u.ID_GRUPO     := grupo_u               ;
         u.ID_CATEGORIA := categoria_u           ;

        if NOT u.GRAVA_ID(erro) then
           begin
              showmessage(erro);
              exit;
           end;

      finally
         u.DisposeOf;

      end;

//  ID_LANCAMENTO:= ID_LANCAMENTO+1;
    cc := '{'+'"lancamento":"'+IntToStr(numerador)+'"'+'}';
    va_05_dm.DM.RESTRequest_contadorX.Body.ClearBody;
    va_05_dm.DM.RESTRequest_contadorX.AddBody(cc,ctAPPLICATION_JSON);
    va_05_dm.DM.RESTRequest_contadorX.Execute;

   end;


//------------------------------------------------------------------------------
procedure Tva_debitos2.FormCreate(Sender: TObject);
 var
  u       : TUsuario ;
  erro    : String   ;
  bb      : integer  ;

 begin

       va_05_dm.DM.RESTRequest_vai.Execute;
 if    va_05_dm.DM.FDMemTable_vai.Locate('id;status',VarArrayOf(['0', 's']), []) then
       begin
           numerador := va_05_dm.DM.FDMemTable_contador.FieldByName('lancamento').AsInteger;
       end;



    try
        u            := TUsuario.Create(va_05_dm.DM.conn) ;
        u.ID_USUARIO := StrToInt(va_abertura.ID_PUBLICO)  ;
        id_usuario   := va_abertura.ID_PUBLICO            ;

        if NOT u.Busca_ID(erro) then
            begin
              exit;
            end;

         ListarCategorias   ;
         PosicionaCategoria ;
         PosicionaGrupo     ;
         carrega            ;
         grupo_u           := u.ID_GRUPO;
         categoria_u       := u.ID_CATEGORIA;

    finally
     // numerador := 0 ;
        u.DisposeOf;
    end;

 end;

//------------------------------------------------------------------------------
procedure Tva_debitos2.FormShow(Sender: TObject);
 var
  u       : TUsuario ;
  erro    : String   ;
  bb      : integer  ;
begin

     try
        u            := TUsuario.Create(va_05_dm.DM.conn) ;
        u.ID_USUARIO := StrToInt(va_abertura.ID_PUBLICO)  ;
        id_usuario   := va_abertura.ID_PUBLICO            ;
        if NOT u.Busca_ID(erro) then
           begin
             exit;
           end;
        PosicionaCategoria ;
        PosicionaGrupo     ;
        carrega            ;
        grupo_u      := u.ID_GRUPO    ;
        categoria_u  :=  u.ID_CATEGORIA;
     finally
         u.DisposeOf;
     end;

//         if va_abertura.ALTERACAO_P = 'A' then Alteracao ;
//         if va_abertura.ALTERACAO_P = 'i' then   Inclusao  ;


end;


procedure Tva_debitos2.ImageA1Click(Sender: TObject);
begin
   SelecionaIcone(TImage(Sender));
end;

procedure Tva_debitos2.Img2_direitaClick(Sender: TObject);
begin
  close;
end;

procedure Tva_debitos2.img_addClick(Sender: TObject);
begin
//    ShowMessage('linha 303');
end;

procedure Tva_debitos2.lbCategoriaItemClick(const Sender: TCustomListBox;
  const Item: TListBoxItem); // pega o índice da categoria no list Box
begin
     categoria_u := lbCategoria.ItemIndex;
end;

procedure Tva_debitos2.SelecionaIcone(img: TImage);
begin
    icone_selecionado  := img.Bitmap;
    indice_selecionado := TListBoxItem(img.Parent).Index;
    img_selecao.Parent := img.Parent;
end;


//------------------------------------------------------------------------------
procedure Tva_debitos2.ListarCategorias;                                  // 1 //
var     //------------------------- C A T E G O R I A --------------------------
  JObject : TJSONObject;
  Pair    : TJSONPair;
  Item    : TJSONObject;
begin

  va_05_dm.DM.FDMemTable_categoria_G.Close;
  va_05_dm.DM.FDMemTable_categoria_G.Open;
  va_05_dm.DM.FDMemTable_categoria_G.EmptyDataSet;
  va_05_dm.DM.RESTReq_categoria_G.Params.Clear;
  va_05_dm.DM.RESTReq_categoria_G.execute;

  JObject := TJSONObject.ParseJSONValue(va_05_dm.DM.RESTReq_categoria_G.Response.Content ) as TJSONObject;

  try
    for Pair in JObject do
    begin  // criei uma tabela a partir do json

      Item := Pair.JsonValue as TJSONObject;
      va_05_dm.DM.FDMemTable_categoria_G.Append;
      va_05_dm.DM.FDMemTable_categoria_G.FieldByName('descricao').AsString := Item.GetValue<string>('descricao');
      va_05_dm.DM.FDMemTable_categoria_G.FieldByName('icone').AsInteger    := Item.GetValue<Integer>('icone');
      va_05_dm.DM.FDMemTable_categoria_G.FieldByName('id').AsString        := Item.GetValue<string>('id');
      va_05_dm.DM.FDMemTable_categoria_G.Post;

    end;
  finally
    JObject.Free;
  end;

    // pego a tabela criada e vou fazer uma veredura nos registros


   with va_05_dm.DM.FDMemTable_categoria_G   do
   begin
     First;
     while Not Eof  do
     begin
       if FieldByName('descricao').AsString <> '' then
          begin
             addcategoria( FieldByName('id').AsInteger, FieldByName('icone').AsInteger,FieldByName('descricao').AsString );
          end;
       Next;
     end ;
   end;

end;


procedure Tva_debitos2.ListView1ItemClick(const Sender: TObject;
  const AItem: TListViewItem); // Carrega o ID do GRUPO
begin

   id_img     := AItem.Objects.FindObjectT<TListItemText>('Text3').Text;
   descricao  := AItem.Objects.FindObjectT<TListItemText>('Text1').Text;
   id         := IntToStr(ListView1.ItemIndex); //----------   erro do id    ---------------
   grupo_u    := StrToInt(AItem.TagString);

end;


procedure Tva_debitos2.AddCategoria(id, icone : integer; descricao: string);   // 2 //
var  //------------ ADICIONA CATEGORA .... no modo RAIS kkkkk
    item    : TListBoxItem;
    rect    : TRectangle;
    lbl     : TLabel;
    img_box : TImage;
begin

//------------------------------------------------------------------------------
    item := TListBoxItem.Create(lbCategoria);
    item.HitTest    := false;
    item.Selectable := True ;
    item.Height     := 60  ;
    item.Width      := 130 ;
    item.Text       := ' ';
    item.Tag        := id; // o que está dentro da variável é o valor do icone
    item.Align      := TAlignLayout.Client;
//------------------------------------------------------------------------------
    rect            := TRectangle.Create(item);
    rect.Cursor     := crHandPoint;
    rect.HitTest    := false;
    rect.Fill.Color := $FFE2E2E2;
    rect.Align      := TAlignLayout.Client;
    rect.Margins.Top    := 8;
    rect.Margins.Left   := 8;
    rect.Margins.Right  := 8;
    rect.Margins.Bottom := 8;
    rect.XRadius        := 10;
    rect.YRadius        := 10;
    rect.Stroke.Kind    := TBrushKind.None;

//------------------------------------------------------------------------------
    img_box         := TImage.Create(rect);
    img_box.Height  := 50  ;
    img_box.Width   := 50 ;

    img_box := TImage.Create(rect);

case icone of
  0: img_box.Bitmap.Assign(ImageA1.Bitmap);
  1: img_box.Bitmap.Assign(ImageA2.Bitmap);
  2: img_box.Bitmap.Assign(ImageA3.Bitmap);
  3: img_box.Bitmap.Assign(ImageA4.Bitmap);

  4: img_box.Bitmap.Assign(ImageB1.Bitmap);
  5: img_box.Bitmap.Assign(ImageB2.Bitmap);
  6: img_box.Bitmap.Assign(ImageB3.Bitmap);
  7: img_box.Bitmap.Assign(ImageB4.Bitmap);

  8: img_box.Bitmap.Assign(ImageC1.Bitmap);
  9: img_box.Bitmap.Assign(ImageC2.Bitmap);
 10: img_box.Bitmap.Assign(ImageC3.Bitmap);
 11: img_box.Bitmap.Assign(ImageC4.Bitmap);

 12: img_box.Bitmap.Assign(ImageD1.Bitmap);
 13: img_box.Bitmap.Assign(ImageD2.Bitmap);
 14: img_box.Bitmap.Assign(ImageD3.Bitmap);
 15: img_box.Bitmap.Assign(ImageD4.Bitmap);

end;

    img_box.Align   := TAlignLayout.Center;   //  <-----------------
    img_box.HitTest := false;                //   <-----------------
//------------------------------------------------------------------------------
    lbl                 := TLabel.Create(rect);
    lbl.HitTest         := false;
    lbl.Align           := TAlignLayout.Bottom;
    lbl.Text            := descricao;
    lbl.TextSettings.HorzAlign := TTextAlign.Center;
    lbl.TextSettings.VertAlign := TTextAlign.Center;
    lbl.StyledSettings         := lbl.StyledSettings - [TStyledSetting.Size, TStyledSetting.FontColor, TStyledSetting.Style, TStyledSetting.Other];
    lbl.Font.Size              := 13;
    lbl.FontColor := $FF3A3A3A;
//------------------------------------------------------------------------------
    rect.AddObject(lbl);
    rect.AddObject(img_box);
    item.AddObject(rect);
    lbCategoria.AddObject(item);

end;


procedure Tva_debitos2.Carrega;   // vRESTResponse transformo em FDMAmTable ,motivo : uso o  RESTRequest .json?orderBy="status"&equalTo="s" .Se tirar , não preciso usar o CARREGA
var   // AQUI DEFINE O GRUPO
  JObject : TJSONObject;
  Pair    : TJSONPair;
  Item    : TJSONObject;
  x       : integer;
  y       : string;
begin

  va_05_dm.DM.FDMemTable_grupo.Close;
  va_05_dm.DM.FDMemTable_grupo.Open;
  va_05_dm.DM.FDMemTable_grupo.EmptyDataSet;
  va_05_dm.DM.RESTRequest_grupo.Params.Clear;
  va_05_dm.DM.RESTRequest_grupo.execute;

  JObject := TJSONObject.ParseJSONValue(va_05_dm.DM.RESTRequest_grupo.Response.Content ) as TJSONObject; // carrega JSON Object

  try
    for Pair in JObject do
    begin   // Estou construindo FDMemTabel com o JSON do Response
      Item := Pair.JsonValue as TJSONObject;
      va_05_dm.DM.FDMemTable_grupo.Append;
      va_05_dm.DM.FDMemTable_grupo.FieldByName('descricao').AsString := Item.GetValue<string>('descricao');
      va_05_dm.DM.FDMemTable_grupo.FieldByName('icone').AsInteger    := Item.GetValue<Integer>('icone');
      va_05_dm.DM.FDMemTable_grupo.FieldByName('id').AsString        := Item.GetValue<string>('id');
      va_05_dm.DM.FDMemTable_grupo.Post;
    end;
    x:=va_05_dm.DM.FDMemTable_grupo.RecordCount; // x tem o número de registros da tabela
  finally
    JObject.Free;
  end;

               va_05_dm.DM.FDMemTable_grupo.First;
     while Not va_05_dm.DM.FDMemTable_grupo.Eof  do
     begin
          addcategoria2(
          va_05_dm.DM.FDMemTable_grupo.FieldByName('descricao').AsString,
          va_05_dm.DM.FDMemTable_grupo.FieldByName('icone').AsString,
          va_05_dm.DM.FDMemTable_grupo.FieldByName('id').AsString
                       );
          va_05_dm.DM.FDMemTable_grupo.Next;
     end;
end;


procedure Tva_debitos2.addcategoria2(var1, var2, var3: string);
begin  // -------------- AQUI E GRUPO --------------------

  if  var1 <> '' then
  begin

    with ListView1.Items.Add do
    begin
       // descrição
       TListItemText(Objects.FindDrawable('Text1')).Text :=  var1;
       TListItemText(Objects.FindDrawable('Text3')).Text :=  var2;
       TagString := var3;
       Tag       := StrToInt(var3);

//     ShowMessage('.541 -var1-['+var1+'] var2 ['+var2+']   var3 ['+var3+']');

       case StrToIntDef(var2, 0) of

         0:TListItemImage(Objects.FindDrawable('Image2')).Bitmap := ImageXA1.Bitmap;
         1:TListItemImage(Objects.FindDrawable('Image2')).Bitmap := ImageXA2.Bitmap;
         2:TListItemImage(Objects.FindDrawable('Image2')).Bitmap := ImageXA3.Bitmap;
         3:TListItemImage(Objects.FindDrawable('Image2')).Bitmap := ImageXA4.Bitmap;
         4:TListItemImage(Objects.FindDrawable('Image2')).Bitmap := ImageXB1.Bitmap;
         5:TListItemImage(Objects.FindDrawable('Image2')).Bitmap := ImageXB2.Bitmap;
         6:TListItemImage(Objects.FindDrawable('Image2')).Bitmap := ImageXB3.Bitmap;
         7:TListItemImage(Objects.FindDrawable('Image2')).Bitmap := ImageXB4.Bitmap;
         8:TListItemImage(Objects.FindDrawable('Image2')).Bitmap := ImageXC1.Bitmap;
         9:TListItemImage(Objects.FindDrawable('Image2')).Bitmap := ImageXC2.Bitmap;
        10:TListItemImage(Objects.FindDrawable('Image2')).Bitmap := ImageXC3.Bitmap;
        11:TListItemImage(Objects.FindDrawable('Image2')).Bitmap := ImageXC4.Bitmap;
        12:TListItemImage(Objects.FindDrawable('Image2')).Bitmap := ImageXD1.Bitmap;
        13:TListItemImage(Objects.FindDrawable('Image2')).Bitmap := ImageXD2.Bitmap;
        14:TListItemImage(Objects.FindDrawable('Image2')).Bitmap := ImageXD3.Bitmap;
        15:TListItemImage(Objects.FindDrawable('Image2')).Bitmap := ImageXD4.Bitmap;
       else
 //        TListItemImage(Objects.FindDrawable('Image2')).Bitmap := Image1.Bitmap;
       end;
    end;
  end;
end;


procedure Tva_debitos2.PosicionaGrupo;
var
  I : Integer;
begin

  for I := 0 to ListView1.Items.Count - 1 do
     begin

       if ListView1.Items[I].Tag = grupo_u then
          begin
            ListView1.ItemIndex := I;
            ListView1.ScrollTo(I);
            Break;
          end;
     end;

end;



procedure Tva_debitos2.Rectangle9DblClick(Sender: TObject);
begin
   if StrToInt(Edit1_VALOR_PAR.Text) > StrToInt(Edit_VALOR.Text) then
      begin
        exit;
      end;
   Edit_VALOR.Text      :='';
   Edit1_VALOR_PAR.Text :='';
end;


procedure Tva_debitos2.RoundRect3Click(Sender: TObject);
var
    l  : TLancamento;
  erro : string;
     x : integer;
begin

    if       va_abertura.ALTERACAO_P = 'i' then
    begin
      if     numerador = 0 then        // 01 - passei e criei uma variavel com o valor 0
      begin
              va_05_dm.DM.RESTRequest_contador_G.Execute;
          if  va_05_dm.DM.FDMemTable_contador.Locate('id;status',VarArrayOf(['0', 's']), []) then
          begin
              numerador := va_05_dm.DM.FDMemTable_contador.FieldByName('lancamento').AsInteger;
              numerador:=numerador+1;
          end;
      end
      else   numerador:=numerador+1;


    end;


 if va_abertura.ALTERACAO_P = 'i' then   resolve;
    AnimationLarg.Inverse  := false;
    AnimationLarg.Start;
    lbl_arc.AnimateFloat('Opacity', 0, 0.4);


   TThread.CreateAnonymousThread(procedure
    begin
      try
         if Edit_VALOR.Text      = '' then Edit_VALOR.Text      := '0';
         if Edit1_VALOR_PAR.Text = '' then Edit1_VALOR_PAR.Text := '0';

         l               := TLancamento.Create(va_05_dm.DM.conn)  ;
         l.VALOR         := StrToFloat(Edit_VALOR.Text)           ;    // VALOR
         l.VALOR2        := StrToFloat(Edit1_VALOR_PAR.Text)      ;    // VALOR2
         l.ID_GRUPO      := grupo_u                               ;    // ID_GRUPO
         l.ID_CATEGORIA  := categoria_u                           ;    // ID_CATEGORIA
         l.DATA          := Date                                  ;    // DATA
         l.DESCRICAO     := Edit_DESCRIAO.Text                    ;    // DESCRIÇÃO
         l.ID_LANCAMENTO := numerador                             ;    // se for ativado agora vai ter problmas com INCLUSAO
         l.ID            := StrToInt(id_usuario)                  ;    // Quem é o USUARIO
         l.ID_X          := va_abertura.ID_LANCAMENTO_P           ;  // O ID_X é exclusivo para alterações
         l.STATUS        := va_abertura.ALTERACAO_P               ;

          if va_abertura.ALTERACAO_P = 'A' then l.Alterar(erro)  else   l.inserir(erro); // Altera ou Inclui

      finally

          l.DisposeOf;
          Edit_VALOR.Text      := '0' ;  // foi retirado o 0,00
          Edit1_VALOR_PAR.Text := '0' ;  // Foi retirado o 0,00
          Edit_DESCRIAO.Text   := '' ;
      end;

         TThread.Synchronize(nil, procedure
      begin
          AnimationLarg.Inverse := true;
          AnimationLarg.Start;
          lbl_arc.Text := 'Novo lançamento';
          lbl_arc.AnimateFloat('Opacity', 1, 0.4);
      end);

    end).Start;

    if NOT Assigned(va_abertura) then
           Application.CreateForm(Tva_abertura, va_abertura);
           Application.MainForm := va_abertura ;
           va_debitos2.Close  ;
           va_abertura.Show  ;
           va_abertura.AtualizarTela ;
end;


procedure Tva_debitos2.PosicionaCategoria;
var
  i : Integer;
begin
   for i := 0 to lbCategoria.Count - 1 do
       begin
          if lbCategoria.ListItems[i].Tag = categoria_u then
              begin
                 lbCategoria.ItemIndex := i;
                 Break;
              end;
       end;
end;

procedure Tva_debitos2.Prepara;
begin

//  if va_abertura.ALTERACAO_P = 'A' then Alteracao ;
//  if va_abertura.ALTERACAO_P = 'i' then   Inclusao  ;

end;


procedure Tva_debitos2.Alteracao;
var
     g: TLancamento;
     erro: string;
begin
     // ShowMessage('667 ESTOU NO ALTERACAO ' );
     g               := TLancamento.Create(va_05_dm.DM.conn2);
     g.ID_LANCAMENTO := StrToInt(va_abertura.ID_LANCAMENTO_P);         //  ID_LANCAMENTO = ID_LANCAMENTO_P
     id_usuario      := va_abertura.ID_PUBLICO ;                       //  quem é o usuario dono do registro

     if NOT g.Busca_ID(erro) then
        begin
           ShowMessage(erro);
           exit;                                                       // com o ID_LANCAMENTO recebo :DESCRIÇÃO, VALOR, GRUPO, CATEGORIA ...
        end;

     grupo_u              :=   g.ID_GRUPO     ;
     categoria_u          :=   g.ID_CATEGORIA ;
     Edit_DESCRIAO.Text   :=   g.DESCRICAO    ;
     Edit_VALOR.Text      := FloatToStr(g.VALOR)  ;
     Edit1_VALOR_PAR.Text := FloatToStr(g.VALOR2) ;
     PosicionaCategoria   ;                                             //  carrega a última categoria escolhida
     ListarCategorias     ;                                             //  Cria a listagem das categorias
     carrega              ;

 end;


procedure Tva_debitos2.Inclusao;
var
  u    : TUsuario ;
  erro : string   ;

 begin

     lbCategoria.Clear;
     carrega;
     va_abertura.ALTERACAO_P := 'i' ;

     u            := TUsuario.Create(va_05_dm.DM.conn) ;
     u.ID_USUARIO := StrToInt(va_abertura.ID_PUBLICO)  ;
     id_usuario   := va_abertura.ID_PUBLICO            ;
     grupo_u      := u.ID_GRUPO                        ;
     categoria_u  := u.ID_CATEGORIA                    ;

     if NOT u.Busca_ID(erro) then
        begin
            exit;
        end;

     grupo_u            :=  u.ID_GRUPO     ;
     categoria_u        :=  u.ID_CATEGORIA ;
     PosicionaCategoria                    ;
     ListarCategorias                      ;
     carrega                               ;
     grupo_u            := u.ID_GRUPO      ;
     categoria_u        := u.ID_CATEGORIA  ;

 end;


procedure Tva_debitos2.resolve;
var
 o_que_vejo : integer ;
 cc         : string  ;
begin
// CONSULTA
       va_05_dm.DM.RESTRequest_vai.Execute;
 if    va_05_dm.DM.FDMemTable_vai.Locate('id;status',VarArrayOf(['0', 's']), []) then
       begin
           o_que_vejo := va_05_dm.DM.FDMemTable_contador.FieldByName('lancamento').AsInteger;
       end;

// GRAVA
      if o_que_vejo < NUMERADOR then
         begin
             va_05_dm.DM.RESTRequest_volta.Params.ParameterByName('x').Value := '0';
             va_05_dm.DM.RESTRequest_volta.Params.Items[0].name  := 'param';                                   // item = param
             cc := '{'+'"lancamento":"'+IntToStr(NUMERADOR)+'"'+'}';
             va_05_dm.DM.RESTRequest_volta.Params.Items[0].value :=cc;                                         // colocado dentro do RESTRequest1 o conteudo de cc
             va_05_dm.DM.RESTRequest_volta.Params.Items[0].ContentType := ctAPPLICATION_JSON;                  // ct application
             va_05_dm.DM.RESTRequest_volta.Params.Items[0].Kind := TRESTRequestParameterKind.pkGETorPOST;      // recebe um pk GET ou POST ( se não existir atualize )
             va_05_dm.DM.RESTRequest_volta.Execute;
         end;
end;


end.


