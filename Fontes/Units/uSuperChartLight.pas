...
type
  ...
  public
    ...
    procedure LoadFromJSON(jsonString: string; var error: string); // voce pode criar uma variavel JOSON e montar o grafico a partir dela {"nome":"lauro"}
...
begin
    ...
    // Default values...
    FShowValues         := true;             // define se os valores vão ser mostrado ou não
    FFontSizeValues     := 12;
    FFontColorValues    := $FF424242; //QUASE PRETO
    FBarColor           := $FF2A8FF7; // AZUL
    FShowBackground     := false;
    FBackgroundColor    := $FFF3F3F3; // BRANCO
    FFormatValues       := '';
    FFontSizeArgument   := 12;
    FFontColorArgument  := $FF424242; // QUASE PRETO   // define qual a cor do argumento no grafico
    FRoundedBotton      := false;
    FRoundedTop         := false;
    FLineColor          := $FF2A8FF7; // AZUL
    ClearChart;
end;
...
procedure TSuperChart.LoadFromJSON(jsonString: string; var error: string); // le uma string  em formato JSON
var
